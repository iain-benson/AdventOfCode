...
int main() {
    ...
    for ( int line = 0 ; line < input.size() ; ++ line ) {
        for ( int col = 0 ; col < input[line].size() ; ++ col ) {
            if ( input[line][col] >= 48 && input[line][col] <= 57 ) {
                
                int numberEnd = input[line].find_first_not_of("0123456789", col) - 1;
                if ( numberEnd == std::string::npos - 1 ) {
                    numberEnd = input[line].size() - 1;
                }
                
                for ( int checkLine = std::max(0, line - 1) ; 
                          checkLine <= std::min((int)input.size() - 1, line + 1) ;
                        ++checkLine ) {

                    for ( int checkCol = std::max(0, col - 1) ;
                              checkCol <= std::min((int)input[checkLine].size() - 1, numberEnd + 1) ;
                            ++checkCol ) {
