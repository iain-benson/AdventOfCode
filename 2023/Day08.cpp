...
int main()
{
    for ( int line = 2 ; line < input.size() ; ++line ) {
        ...
        std::sregex_token_iterator iter(input[line].begin(), input[line].end(), space, -1);
        std::vector<std::string> split(iter, end);
        
        tree.emplace(std::make_pair(split[0], std::make_pair(split[1], split[2])));
    }
    ...
    while ( location != "ZZZ" ) {
        switch (input[0][step++ % input[0].length()]) {
            case 'L':
                location = tree[location].first;
                break;
            case 'R':
                location = tree[location].second;
                break;
            default:
                return -1;
        }
    ...
    transform_if( tree.begin(), tree.end(), std::back_inserter(locations),
        [] (const auto& loc) {
            return loc.first;
        }, [] (const auto& loc) {
            return (loc.first[loc.first.length() - 1] == 'A');
        } );
    ...
    std::cout << "Part 2 Steps: " << lcm << std::endl;
