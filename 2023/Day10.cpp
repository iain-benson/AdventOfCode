...
int main() {
    ...
    std::vector<std::vector<std::pair<std::pair<int,int>, std::pair<int,int>>>> paths;
    std::vector<std::vector<int>> distances;

    paths.resize(input.size());
    ...
    for (auto line : input) {
        ...
        paths[y].resize(line.length());
        distances[y].resize(line.length());
        int x = 0;
        for (auto cell: line) {

            switch (cell) {
                case '|':
                    paths[y][x].first = std::make_pair(x, y-1);
                    paths[y][x].second = std::make_pair(x, y+1);
                    break;
                case '-':
                    paths[y][x].first = std::make_pair(x-1, y);
                    paths[y][x].second = std::make_pair(x+1, y);
                    break;
                case 'L':
                    paths[y][x].first = std::make_pair(x, y-1);
                    paths[y][x].second = std::make_pair(x+1, y);
                ...
                case 'F':
                    ...
                    break;
                case '.':
                    paths[y][x].first = std::make_pair(x, y);
                    paths[y][x].second = std::make_pair(x, y);
                    break;
                case 'S':
                    paths[y][x].first = std::make_pair(x, y);
                    paths[y][x].second = std::make_pair(x, y);
...
/*
    for ( auto line : paths ) {
        for ( auto cell : line ) {
            ...
            std::cout << "(" << cell.second.first << "," << cell.second.second << "),";
        }
        std::cout << std::endl;
    }
*/
    for ( int direction = 0 ; direction < 4 ; ++direction ) {
        ...
        while ( ( x2 != start.first || y2 != start.second ) && !invalid ) {

            std::cout << "(" << x2 << "," << y2 << ") -> ";

            if ( y2 < 0 || y2 >= paths.size() || x2 < 0 || x2 >= paths[y2].size() ) {

                std::cout << "* - Off the map";
                invalid = true;

            } else if ( paths[y2][x2].first.first == x && paths[y2][x2].first.second == y ) {
        ...
        if ( !invalid ) {
            std::cout << "Distance = " << ( distance + 1) / 2 << std::endl;
            break;
        }
    }
}
