...
int main() {
    ...
    for ( auto game : input ) {
        ...
        std::regex colon(":");
        std::sregex_token_iterator iter1(game.begin(), game.end(), colon, -1);
        std::sregex_token_iterator end;
        std::vector<std::string> split(iter1, end);
    
        std::regex space(" ");
        std::sregex_token_iterator iter2(split[0].begin(), split[0].end(), space, -1);
        std::vector<std::string> split2(iter2, end);

        int gameNum = atoi(split2[1].c_str());

        std::regex semicolon(";");
        std::sregex_token_iterator iter3(split[1].begin(), split[1].end(), semicolon, -1);
        std::vector<std::string> split3(iter3, end);

        for ( auto draw: split3 ) {
            ...
            std::regex comma(",");
            std::sregex_token_iterator iter4(draw.begin(), draw.end(), comma, -1);
            std::vector<std::string> split4(iter4, end);
            
            for ( auto colour: split4 ) {

                std::sregex_token_iterator iter5(colour.begin(), colour.end(), space, -1);
                std::vector<std::string> split5(iter5, end);
                
                if ( split5[2] == "blue" ) 
                    totalBlue += atoi(split5[1].c_str());
                else if ( split5[2] == "red" ) 
                    totalRed += atoi(split5[1].c_str());
                else if ( split5[2] == "green" ) 
                    totalGreen += atoi(split5[1].c_str());
            }
            
            maxBlue = std::max(maxBlue, totalBlue);
            maxRed = std::max(maxRed, totalRed);
            maxGreen = std::max(maxGreen, totalGreen);
        }

        if ( maxBlue <= allowedBlue && maxRed <= allowedRed && maxGreen <= allowedGreen ) {
            validTotal += gameNum;
        }

        powerTotal += maxBlue * maxRed * maxGreen;

        // std::cout << gameNum << ": " << "Blue: " << maxBlue << ", Red: " << maxRed << ", Green: " << maxGreen << std::endl;
    }
    
    std::cout << "Valid Games Total " << validTotal << std::endl;
    std::cout << "Power Total " << powerTotal << std::endl;
