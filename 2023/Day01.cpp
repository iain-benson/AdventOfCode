...
int main() {
    ...
    std::cout << std::accumulate(input.begin(), input.end(), 0, [&](int total, const auto& line) {

        int firstNumPos = line.size();
        int lastNumPos = 0;
        ...
        for ( auto number: numbers ) {
            auto pos = line.find(number);
            if (pos != std::string::npos && pos <= firstNumPos) {
                firstNumPos = pos;
                firstNumber = value % 10;
            }
            
            pos = line.rfind(number);
            if (pos != std::string::npos && pos >= lastNumPos) {
                lastNumPos = pos;
                lastNumber = value % 10;
            }
            ++value;
