...
auto cmp = [](const std::string&lhs, const std::string& rhs) { 
    ...
    lhsMaxCount += lhsJokers;
    rhsMaxCount += rhsJokers;
...
std::map<std::string, int, decltype(cmp)> hands;
...
int main()
{
    ...
    for ( auto line : input ) {
        ...
        std::sregex_token_iterator iter(line.begin(), line.end(), space, -1);
        std::vector<std::string> split(iter, end);

        hands.emplace(std::make_pair(split[0], atoi(split[1].c_str())));
    }
    
    int total = 0;
    int rank = 1;
    for ( auto hand : hands ) {
        std::cout << rank << ": " << hand.first << " " << hand.second << std::endl;
        total += hand.second * rank++;
    }
